...
export default function AcceptInvitePage() {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    setLoading(true);
    try {
      const res = await fetch("/api/auth/accept-supplier-invite", {
        method: "POST",
      ...
      <div className="min-h-screen flex items-center justify-center p-6">
        <Card className="w-full max-w-md">
          <CardHeader>
            ...
            <p className="text-muted-foreground text-sm">This link is missing the invitation token. Please use the link from your email.</p>
          </CardHeader>
          <CardContent>
            <Link href="/login">
    ...
    <div className="min-h-screen flex items-center justify-center p-6">
      <Card className="w-full max-w-md">
        <CardHeader className="text-center">
          ...
          <h2 className="text-xl font-bold">Accept portal invitation</h2>
          <p className="text-muted-foreground text-sm">Set your password to access the portal</p>
        </CardHeader>
        <CardContent>
